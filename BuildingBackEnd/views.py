...
class AreaFilterQueryset(BaseFilterBackend):
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        if min is None and max is None:
            ...
            print(max)
            return list(filter(lambda x: x["geom"].area <= max, values))
        elif max is None:
            print(min)
            return list(filter(lambda x: min <= x["geom"].area, values))
        else:
            print(min, max)
            return list(filter(lambda x: min <= x["geom"].area <= max, values))
